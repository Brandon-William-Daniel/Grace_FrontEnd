...
const Profile = () => {
  ...
  // Matt: cart should be in context if used in multiple
  // components
  ...
  async function getPastCart() {
    try {
      const data = await fetch(
        `https://gg-3pln.onrender.com/api/orders/pastcart`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="mainPage">
      ...
      <div>
        {user && user.username ? (
          ...
          "Must login or register to continue"
        )}
      </div>
      <div>
        <h1>Current Cart:</h1>
        ...
        {current && current.length
          ? current.map((el) => {
              return (
                <div className="productsDiv" key={el.id}>
          ...
          : "There is currently nothing to be displayed"}
